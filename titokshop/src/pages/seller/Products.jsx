...
export default function SellerProducts() {
  ...
  const tabs = [
    ...
    {name:"Đã ẩn 22",value:2},
    {name:"Đã xóa 12",value:3},
  ];
  ...
  const getProducts = () => {
    api
      .get(`/sale/product/findall?${filters.status!=null?`status=${filters.status}`:""}&${filters.categoriesId!=null?`categories=${filters.categoriesId}`:""}&${filters.minPrice!=null?`minPrice=${filters.minPrice}`:""}&${filters.maxPrice!=null?`maxPrice=${filters.maxPrice}`:""}&key=${filters.key}&isLive=${filters.isLive}`)
      .then((v) => {
    ...
    <div className="bg-gray-50 min-h-screen p-6 text-sm text-gray-800">
      ...
      <div className="bg-white p-4 rounded border mb-6 shadow-sm">
        <div className="flex flex-wrap gap-3 mb-3">
          ...
          >
            <CategorySelector setIfEnd={false} setCategory={(cat)=>{
          ...
          <button className="border rounded px-4 py-2 text-gray-700 hover:bg-gray-100">
            Đặt lại
          </button>
        </div>
        <div className="flex gap-2 items-center mt-2">
          <span className="text-gray-800 mr-2 font-sans font-medium">Đã chọn: 1/2</span>
          <button onClick={()=>{
      ...
      {sampleProducts.content.map((v) => (
        <ProductItem key={v.id} product={v} />
      ))}
    </div>
  );
}
