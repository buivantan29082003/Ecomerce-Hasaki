...
public interface VoucherRepository extends JpaRepository<VoucherShop, Integer>, JpaSpecificationExecutor<VoucherShop> {
    ...
    @Query("""
                ...
                FROM VoucherShop v
                WHERE v.shop.id=:shopId and (v.voucherName LIKE :key) AND (:ids is null or v.id in :ids)
                  AND (
                    :status IS NULL
    ...
    Page<VoucherInfo> findAllParentOnly(
            ...
            Pageable pageable, @Param("ids") List<Integer> l, @Param("shopId") Integer shopId);
    ...
    @Modifying
    @Query("UPDATE VoucherShop p set p.isActive=:isActive where p.id=:voucherId")
